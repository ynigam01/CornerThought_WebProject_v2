...
async function embedLessonOnComplete(supabase, lessonId, organizationId) {
    ...
    if (highLevelTitle) {
        try {
            ...
            const { error } = await supabase
                ...
                .eq('organization_id', organizationId);
            if (error) {
                console.error('embedLessonOnComplete: high_level_title update failed:', error.message);
            }
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
    ...
    for (const res of [causesRes, impactsRes, actionsRes, fpcsRes, metadataRes]) {
        if (res.error) {
            console.error('embedLessonOnComplete: failed loading secondary rows:', res.error.message);
        }
    }

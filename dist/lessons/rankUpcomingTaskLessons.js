...
exports.rankUpcomingTaskLessons = rankUpcomingTaskLessons;
 ...
 */
async function rankUpcomingTaskLessons(supabase, req) {
    ...
    const taskEmbedding = (0, cosineSimilarity_1.parseEmbedding)(listRow.search_embedding);
    if (!taskEmbedding) {
    ...
    let sourceProjectTypeEmbedding = null;
    if (sourceProject && sourceProject.project_type_id != null) {
        ...
        if (ptError) {
            ...
            sourceProjectTypeEmbedding = (0, cosineSimilarity_1.parseEmbedding)(ptRow?.search_embedding);
        }
    }
    ...
    if (sourceDetailsError) {
        console.error('rankUpcomingTaskLessons: source project_details load failed:', sourceDetailsError.message);
    }
    ...
    const bestMetaByLessonId = new Map();
    let from = 0;
    while (true) {
        ...
        for (const row of page) {
            if (row.lessons_learned_id == null)
                continue;
            const emb = (0, cosineSimilarity_1.parseEmbedding)(row.search_embedding);
            if (!emb)
                continue;
            const sim = (0, cosineSimilarity_1.cosineSimilarity)(taskEmbedding, emb);
            if (sim < METADATA_THRESHOLD)
                continue;
            const key = String(row.lessons_learned_id);
            const prev = bestMetaByLessonId.get(key);
            if (prev == null || sim > prev)
                bestMetaByLessonId.set(key, sim);
        }
        if (page.length < META_PAGE_SIZE)
            break;
        from += META_PAGE_SIZE;
    }
    ...
    const projectIds = Array.from(new Set(Array.from(lessonsById.values())
        .map((l) => l.projectId)
        .filter((id) => id != null)));
    ...
    if (sourceDetailEmbeddings.length > 0 && projectIds.length > 0) {
        for (const chunk of chunkIds(projectIds, 50)) {
            const { data: details, error } = await supabase
                ...
                .not('search_embedding', 'is', null);
            if (error) {
                console.error('rankUpcomingTaskLessons: candidate project_details batch failed:', error.message);
                continue;
            }
    ...
    for (const [lessonKey, metadataSimilarity] of bestMetaByLessonId.entries()) {
        ...
        if (sourceProjectTypeEmbedding && lesson.projectId != null) {
            ...
            if (typeId != null) {
                ...
                if (candTypeEmb) {
                    projectTypeScore = (0, cosineSimilarity_1.cosineSimilarity)(sourceProjectTypeEmbedding, candTypeEmb);
                }
            }
        }
        const candDetails = lesson.projectId != null
    ...
    results.sort((a, b) => b.score - a.score || b.metadataSimilarity - a.metadataSimilarity);
    return { results: results.slice(0, MAX_RESULTS) };
}
//# sourceMappingURL=rankUpcomingTaskLessons.js.map
// test-lesson-search.js
// Quick script to test semantic lesson search from Node
...
async function runTestSearch(queryText) {
  ...
  // 2) Call the match_lesson_search function in Postgres
  const { data, error } = await supabase.rpc('match_lesson_search', {
  ...
  data.forEach((row, index) => {
    console.log(`\n#${index + 1} lesson_id=${row.lesson_id}`);
    console.log(`  score:      ${row.score.toFixed(3)} (similarity: ${row.similarity.toFixed(3)})`);
    console.log(`  metadata match: ${row.has_metadata_match}, text match: ${row.has_text_match}`);
    console.log('  snippet:');
...
const query = process.argv.slice(2).join(' ') || 'corrosion';
